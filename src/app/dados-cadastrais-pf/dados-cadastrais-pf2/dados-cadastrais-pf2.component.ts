...
@Component({
    selector: 'app-dados-cadastrais-pf2',
    templateUrl: './dados-cadastrais-pf2.component.html',
    styleUrls: ['./dados-cadastrais-pf2.component.css']
})
export class DadosCadastraisPf2Component implements OnInit {
            ...
            .subscribe(
                (data: any) => {
                    ...
                    if (message.Codigo === '002') {
                        ...
                        this.router.navigate(['../dados-cadastrais-pf4'], { relativeTo: this.route });
    ...
    consultaDados() {
        this.consultaDadosService.consultaDados()
            .subscribe(
                (response: any) => {
                    const consultar = response.response.dsDadosProdutor.dsDadosProdutor.ttDadosProdutor[0];
    ...
    voltar(idTab: any) {
        this.router.navigate(['../dados-cadastrais-pf'], { relativeTo: this.route });
    ...
    cadastraContato() {
        ...
        if (errors.length > 0) {
            ...
            this.salvarDadosService.salvarContatos(this.salvar)
                .subscribe(
                    (response: any) => {
                        const salvar = response.response.dsDadosProdutor.dsDadosProdutor;
